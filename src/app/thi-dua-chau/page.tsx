...
// TVV90: TVV có thời gian làm việc đến hiện tại không quá N tháng VÀ phải có ít nhất 1 tháng đạt tổng IP >= threshold
function isTVV90Agent(contracts: Contract[], agentCode: string, maxMonths: number = 3, minIP: number = 12_000_000): boolean {
  const agentContract = contracts.find(c => c.agentCode === agentCode);
  if (!agentContract?.startDate) return false;
  const start = new Date(agentContract.startDate);
  const now = new Date();
  const diffMonths = (now.getFullYear() - start.getFullYear()) * 12 + (now.getMonth() - start.getMonth());
  if (diffMonths > maxMonths) return false;
  // Check at least 1 month with total IP >= minIP
  const agentContracts = contracts.filter(c => c.agentCode === agentCode && c.issueDate);
  const monthFYP = new Map<string, number>();
  for (const c of agentContracts) {
    const month = new Date(c.issueDate).toISOString().slice(0, 7);
    monthFYP.set(month, (monthFYP.get(month) || 0) + c.fyp);
  }
  for (const [, totalFYP] of monthFYP) {
    if (totalFYP >= minIP) return true;
  }
  return false;
}
...
function calculateLuot(contracts: Contract[], luotThreshold: number, conditionType: ConditionType, tvv90MaxMonths?: number, tvv90MinIP?: number): number {
  ...
  for (const [agentCode, monthMap] of agentMonthFYP) {
    ...
    if (conditionType === 'activity_round_tvv90') {
      if (!isTVV90Agent(contracts, agentCode, tvv90MaxMonths, tvv90MinIP)) continue;
    }
...
const ContestPoster = React.memo(function ContestPoster({ contestTitle, startDate, endDate, conditionType, targetType, sortedTiers, filteredContracts, groupedData, totalFYP, totalBonus, achievedCount, notAchievedCount, formatCurrency: fc, formatNumber: fn, formatDate: fd, isPreview = false, variant = 'gradient' }: {
  ...
  if (isWhite) {
    return (
      <div className="rounded-2xl border-2 border-emerald-300 bg-emerald-800 overflow-hidden">
          ...
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                ...
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold bg-emerald-600 text-white"><Target className="w-3.5 h-3.5" />{conditionLabel}</span>
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold bg-sky-600 text-white">
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl ${isPreview ? 'border-2 border-emerald-500' : ''}`}>
...
export default function ThiDuaPage() {
  ...
  const nydData: NYDData[] = useMemo(() => {
    ...
    for (const [nydCode, nyd] of nydMap) {
      ...
      if (isActivityRoundMode(conditionType)) {
        ...
        nyd.recruitCount = calculateLuot(recruitedContracts, luotThreshold, conditionType, tvv90MaxMonths, tvv90MinIP);
    ...
    if (targetType === 'nhom') {
      if (isActivityRoundMode(conditionType)) {
        ...
        for (const g of phase1Grouped.values()) {
          const rounds = calculateLuot(g.contracts, luotThreshold, conditionType, tvv90MaxMonths, tvv90MinIP);
      ...
      for (const r of recruiterList) {
        ...
        if (isActivityRoundMode(conditionType)) {
          recruitCount = calculateLuot(recruited, luotHDThreshold, conditionType, tvv90MaxMonths, tvv90MinIP);
          ...
          for (const [, af] of agentFYPMap) { if (af >= luotHDThreshold) recruitCount++; recruitFYP += af; }
        }
  ...
  }, [usePhase2, phase2StartDate, displayContracts, targetType, conditionType, bonusTiers, bonusTiers2, includeIndividualTN, isTVVmMode(conditionType), conditionType === 'activity_round_tvv90', recruiterList, calculateBonusWithTiers, calculateActivityRoundBonusWithTiers, getBonusAmountWithTiers, luotHDThreshold, luotHDCTThreshold, tvv90MaxMonths, tvv90MinIP]);
  ...
  const getEntityValue = useCallback((contracts: Contract[]): number => {
    ...
    return contracts.reduce((sum, c) => sum + c.fyp, 0);
  }, [conditionType]);
  ...
  const handleCopyText = () => {
    ...
    if (targetType === 'nyd') {
      ...
      }).sort((a, b) => b.contract.fyp - a.contract.fyp).forEach(({ contract: c, tier, phaseInfo }, idx) => {
        if (usePhase2 && phase2StartDate) {
    ...
    if (targetType === 'nyd') {
      ...
      if (isPerContractMode(conditionType)) {
        if (usePhase2) {
          ...
          }).sort((a, b) => b.c.fyp - a.c.fyp).map(({ c, tier, phaseInfo }, idx) => {
          ...
          rows = [...displayContracts].map((c) => { const { tier } = calculateBonus(c.fyp); return { c, tier }; }).sort((a, b) => b.c.fyp - a.c.fyp).map(({ c, tier }, idx) => {
            const base: (string | number)[] = [idx + 1, c.nhom || c.maNhom, c.agentCode, c.agentName, formatDate(c.effectiveDate), c.fyp];
            if (useSecondaryCondition && secondaryAFYPMin > 0) base.push(c.afyp);
            if (showRateColumn) base.push(tier ? formatRate(tier) : '');
  ...
  const stats = useMemo(() => {
    ...
    const totalValue = isTotalModeType ? (conditionType === 'total_afyp' ? displayContracts.reduce((sum, c) => sum + c.afyp, 0) : totalFYPValue) : 0;
    ...
    <div className="min-h-screen">
      ...
      <Dialog open={isResultDialogOpen} onOpenChange={(open) => { setIsResultDialogOpen(open); if (!open) setIsResultExpanded(false); }}>
        <DialogContent className={`${isResultExpanded ? 'sm:max-w-5xl max-h-[95vh]' : 'sm:max-w-2xl max-h-[67vh]'} overflow-y-auto bg-white border-emerald-200 p-0 transition-all duration-300`}>
          {/* Action bar */}
          <div className="sticky top-0 z-10 bg-white border-b border-emerald-100 px-3 py-2 flex items-center justify-between">
            <DialogTitle className="text-emerald-700 text-base font-bold flex items-center gap-2">
          ...
          <div ref={resultContentRef} className="px-3 py-2">
            <div ref={printRef}>
              ...
              {/* Result Table */}
              <div className="overflow-x-auto border border-emerald-200 shadow-sm mt-3">
                <Table className="text-xs">
                      ...
                      {targetType === 'nyd' ? (
                        <>
                          ...
                          {usePhase2 ? (
                            ...
                            <TableHead className="text-white min-w-[65px] font-bold text-center bg-emerald-800">
                              <div className="flex items-center justify-center gap-1"><Sparkles className="w-3 h-3" /> Thưởng</div>
                            </TableHead>
                          )}
                  ...
                  <TableBody>
                    {targetType === 'nyd' ? nydResultRows.map(({ nyd, tier, value }, idx) => {
                      ...
                      return (
                        <TableRow key={nyd.nydCode} className={`${tier ? 'bg-white' : 'bg-red-100'} hover:bg-emerald-50 border-b border-gray-100`}>
                          ...
                          {usePhase2 && phaseBonus ? (
                            ...
                            <TableCell className="text-right bg-emerald-50/80">{tier ? <span className="flex items-center justify-end gap-1">{tier.bonusType === 'gift' ? <Gift className="w-4 h-4 text-pink-500" /> : <Award className="w-4 h-4 text-amber-500" />}<span className="font-bold text-emerald-700 text-sm">{formatBonusAmount(tier, value, nyd.recruitCount)}</span></span> : <span className="text-gray-300 text-xs">—</span>}</TableCell>
                          )}
                      ...
                      if (hideNotAchieved && !tier) return null;
                      return (
                        <TableRow key={group.maNhom} className={`${tier ? 'bg-white' : 'bg-red-100'} hover:bg-emerald-50 border-b border-gray-100`}>
                          <TableCell className="text-center text-gray-500 text-xs">{idx + 1}</TableCell>
                          ...
                          {usePhase2 ? (
                            ...
                            <TableCell className="text-right bg-emerald-50/80">{tier ? <span className="flex items-center justify-end gap-1">{tier.bonusType === 'gift' ? <Gift className="w-4 h-4 text-pink-500" /> : <Award className="w-4 h-4 text-amber-500" />}<span className="font-bold text-emerald-700 text-sm">{formatBonusAmount(tier, group.totalFYP, group.activityRounds)}</span></span> : <span className="text-gray-300 text-xs">—</span>}</TableCell>
                          )}
                    ...
                    }).sort((a, b) => b.contract.fyp - a.contract.fyp).map(({ contract, tier, remaining, phaseInfo }, idx) => {
                      if (hideNotAchieved && !tier) return null;
                      return (
                        <TableRow key={contract.id} className={`${tier ? 'bg-white' : 'bg-red-100'} hover:bg-emerald-50 border-b border-gray-100`}>
                          <TableCell className="text-center text-gray-500 text-xs">{idx + 1}</TableCell>
                          ...
                          {usePhase2 ? (
                            ...
                            <TableCell className="text-right bg-emerald-50/80">{tier ? <span className="flex items-center justify-end gap-1">{tier.bonusType === 'gift' ? <Gift className="w-4 h-4 text-pink-500" /> : <Award className="w-4 h-4 text-amber-500" />}<span className="font-bold text-emerald-700 text-sm">{formatBonusAmount(tier, contract.fyp)}</span></span> : <span className="text-gray-300 text-xs">—</span>}</TableCell>
                          )}
                      ...
                      return tvvTotalRows.map(({ agent, value, tier, remaining, phaseInfo }, idx) => {
                        if (hideNotAchieved && !tier) return null;
                        return (
                          <TableRow key={agent.agentCode} className={`${tier ? 'bg-white' : 'bg-red-100'} hover:bg-emerald-50 border-b border-gray-100`}>
                            <TableCell className="text-center text-gray-500 text-xs">{idx + 1}</TableCell>
                            ...
                            {usePhase2 ? (
                              ...
                              <TableCell className="text-right bg-emerald-50/80">{tier ? <span className="flex items-center justify-end gap-1">{tier.bonusType === 'gift' ? <Gift className="w-4 h-4 text-pink-500" /> : <Award className="w-4 h-4 text-amber-500" />}<span className="font-bold text-emerald-700 text-sm">{formatBonusAmount(tier, value)}</span></span> : <span className="text-gray-300 text-xs">—</span>}</TableCell>
                            )}
                            <TableCell>{!tier && remaining !== null ? <span className="text-[10px] italic text-gray-400">Cần thêm {formatNumber(remaining)}</span> : !tier ? <span className="text-[10px] italic text-gray-400">Chưa đạt</span> : null}</TableCell>
                          </TableRow>
                        );
