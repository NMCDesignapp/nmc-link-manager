...
export default function QuanLyPage() {
  ...
  // Lượt HĐ = đếm số dòng hợp đồng có pdt10DT (IP) >= 3,000,000
  ...
  const renderThuongTuyenNgang = () => {
    ...
    const getMonthStats = (
      ...
      // FYP tháng N = tổng IP của TẤT CẢ TVV trong team + TTN (luôn tính)
      // Theo user: FYP = cá nhân TTN + FYP của tất cả TVV do TTN tuyển dụng
      const allTeamCodes = new Set(teamTVVs.map(tvv => tvv.agentCode));
      let fyp = monthContracts
        .filter(c => allTeamCodes.has(c.agentCode))
        .reduce((s, c) => s + c.pdt10DT, 0);
      // Luôn tính cá nhân TTN vào FYP
      fyp += tnMonthIP;
    ...
    const getCumStats = (
      ...
      // FYP cumulative = tổng IP trong window của TẤT CẢ TVV team + TTN (luôn tính)
      // Theo user: FYP = cá nhân TTN + FYP của tất cả TVV do TTN tuyển dụng
      const allTeamCodesCum = new Set(teamTVVs.map(t => t.agentCode));
      let fyp = windowContracts
        .filter(c => allTeamCodesCum.has(c.agentCode))
        .reduce((s, c) => s + c.pdt10DT, 0);
      // Luôn tính cá nhân TTN vào FYP
      fyp += windowContracts
        .filter(c => c.agentCode === tnCode)
    ...
    <div className="h-screen flex flex-col fixed inset-0 z-50" style={{ backgroundColor: 'transparent' }}>
      ...
      <header className="border-b border-emerald-700/50 backdrop-blur-md px-2 sm:px-4 py-2 flex items-center gap-2 sm:gap-3 flex-shrink-0" style={{ backgroundColor: 'rgba(26, 35, 50, 0.85)' }}>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          {/* Tải Excel — chỉ hiện khi đang xem chính sách (activeSheet='report' && policyOpen). HIỆN cho cả end-user. */}
          {activeSheet === 'report' && policyOpen && (
          ...
          {/* Tải Excel — Tôn vinh: tải toàn bộ 4 bảng Top 5 (H1) về 1 file Excel. HIỆN cho cả end-user. */}
